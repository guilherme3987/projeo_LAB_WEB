...
btn.addEventListener("click",(event) => {
    ...
    if((testeNome === " ")   || (testeNome.lenght<3) || (testeCell === " " ) || (testeCell < 8 ) || ((testeEmail === " ") || !(EmailValidacao(testeEmail)))){
        alert("Preencha os campos corretamente");
        return false;
    }
    else{
        alert("Dados enviados");
        window.location.href = "calculopag.html";
...
botaoProximo.addEventListener("click", () => {
    if (posicaoAtual < 5) {
      posicaoAtual++;
      carrossel.style.transform = `translateX(-${posicaoAtual * 350}px)`;
    }
});
